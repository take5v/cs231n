...
class TensorflowTwoLayerNet(object):
    """
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        X_t = tf.placeholder(tf.float32, shape=(N, D))
        ...
        # shape (N, C).                                                             #
        #############################################################################
        Y1 = tf.maximum(0, tf.matmul(X_t, W1) + b1)
        Y2 = tf.matmul(Y1, W1) + b2

        with tf.Session() as sess:
            sess.run(tf.global_variables_initializer())
            losses = []
            scores = sess.run([Y2], feed_dict={
                x: X
            })
    ...
    def predict(self, X):
        ...
        ###########################################################################
        Y1 = np.maximum(0, X.dot(self.params['W1']) + self.params['b1'])
        ...
        exp_scores = np.exp(scores)
        sum_exp_scores = np.sum(exp_scores, axis=1)
        normalized_scores = exp_scores / sum_exp_scores[:, np.newaxis]

        y_pred = np.argmax(normalized_scores, axis=1)
        ###########################################################################
        #                              END OF YOUR CODE                           #
        ###########################################################################
